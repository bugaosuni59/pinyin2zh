...
*/
set<string>word;
unordered_map<string,ll>P1;// 单字次数
unordered_map<string,ll>P2;// 字1字2次数 
unordered_map<string,ll>P3;// 字1字2字3次数 
unordered_map<string,double>PP1;// log单字频率
unordered_map<string,double>PP2;// log字1字2频率
unordered_map<string,double>PP3;// log字1字2字3频率
...
void process(){
	...
	sum=0;
	for(unordered_map<string,ll>::iterator i=P3.begin();i!=P3.end();i++)
		sum+=i->second;
	for(unordered_map<string,ll>::iterator i=P3.begin();i!=P3.end();i++)
		PP3[i->first]=log(i->second*1.0/sum);
	{
	...
	{
		ofstream ofs("data3.txt");
		for(unordered_map<string,double>::iterator i=PP3.begin();i!=PP3.end();i++)
			ofs<<i->first<<" "<<i->second-PP2[i->first.substr(0,4)]<<endl;
	}
}
void readData(){
	// 读训练数据 
	for(int t=0;t<fnames.size();t++){
//		ifstream ifs("o_"+fnames[t]);
